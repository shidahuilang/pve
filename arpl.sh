...
if [ -f /tmp/boot/user-config.yml ]; then
    ...
    sleep 1s
    SN=$(grep "sn:" /tmp/boot/user-config.yml)
	MAC=$(grep "mac1:" /tmp/boot/user-config.yml)
    echo -e "$green检测到当前序列号为$SN..$plain"
	echo -e "$green检测到当前mac1为$MAC..$plain"
else 
...
read -p "请输入新的mac1(按回车键确定)    " NEWMAC
echo -e "$green新mac1为$NEWMAC..$plain"
echo -e "$green正在替换mac1..$plain"
sleep 1
