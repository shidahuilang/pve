...
const cmad = decodeURIComponent(atob('VEc6aHR0cHM6Ly90Lm1lL2RhaHVpbGFuZzg4OA=='));
/**
 * @param {string} userID
...
async function 生成配置信息(userID, hostName, sub, UA, RproxyIP, _url, env) {
	if (sub) {
		...
		if ((addresses.length + addressesapi.length + addressesnotls.length + addressesnotlsapi.length + addressescsv.length) == 0){
			...
			addresses = addresses.concat('127.0.0.1:1234#大灰狼自用,勿传播,无限流量中');
			if (hostName.includes(".workers.dev")) {
				addressesnotls = addressesnotls.concat(cfips.map(cidr => generateRandomIPFromCIDR(cidr) + '#大灰狼随机节点'));
			} else {
				addresses = addresses.concat(cfips.map(cidr => generateRandomIPFromCIDR(cidr) + '#大灰狼随机节点'));
			}
		}
	}
